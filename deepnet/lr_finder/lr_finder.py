...
class LRFinder(object):
    ...
    def range_test(
        ...
        Arguments:
            ...
            iterations (int): Number of iterations if trained for iterations. Default: None
            epochs (list):
                epochs[0] : Number of epochs if trained for epochs.  Default: None
                epochs[1] : True if data to be stored per epoch, False if data to be stored per iterations of epoch.  Default: False
        ...
        if iterations:
            ...
            if epochs[1]:
                ...
                num_iter = epochs[0]*len(train_loader)
        ...
        if iterations:
            ...
            compare_loss = []
            for iteration in tqdm(range(epochs[0])):
                # Train on batch and retrieve loss
                loss, accuracy = self._train_epoch(train_loader, accumulation_steps, lr_schedule, not epochs[1])
                ...
                if iteration == 0:
                    ...
                    if smooth_f > 0:
                        loss = smooth_f * loss + (1 - smooth_f) * compare_loss[-1]
                    if loss < self.best_loss:
                        self.best_loss = loss
    ...
    def _train_epoch(self, data_loader, accumulation_steps, lr_schedule, if_iteration_wise):
        self.model.train()
        loss_val = 0
        correct = 0
        processed = 0
        for data, target in data_loader:
            ...
            if if_iteration_wise:
                self.history['accuracy'].append((100.0*correct)/processed)
                
        return loss, (100.0*correct)/processed
    ...
    def cyclic_lr(self, max_lr, min_lr, iterations, step_size):
        """Plots a Cyclic Learnig Rate Triangular schedule
        Arguments:
            max_lr (float): Maximum learning rate
            min_lr (float): Mininum learning rate, usually max_lr/10 or max_lr/8
            iterations (int): Number of iterations
            step_size (int): Number of iterations in one epoch
        """
        lr = []
        for i in range(iterations):
            cycle = np.floor(1 + i/(2*step_size))
            x = np.abs(i/step_size - 2*cycle + 1)
            x = min_lr + (max_lr - min_lr)*(1-x)
            lr.append(x)

        plt.rcParams["figure.figsize"] = (12,3)
        plt.plot(lr)
        plt.axhline(y=max_lr, xmin = 0.02, xmax = 0.98, color='g', linestyle='-')
        plt.axhline(y=min_lr, xmin = 0.02, xmax = 0.98, color='g', linestyle='-')

        v = [['lr_max',(0,max_lr),(0,10)],['lr_min', (0,min_lr), (0,-10)]]
        for x in v:
            plt.annotate(x[0], # this is the text
                            (x[1][0], x[1][1]), # this is the point to label
                            textcoords="offset points", # how to position the text
                            xytext=(x[2][0], x[2][1]), # distance from text to points (x,y)
                            ha='center')
        ...
        plt.margins(y=0.2)
        plt.tight_layout()
        plt.savefig('cyclic_lr.png')

    def plot(self, skip_start=0, skip_end=0, log_lr=True, show_lr=None, ax=None, plot_metric = None):
        ...
        if plot_metric == 'Loss':
            metric_value = min(losses)
            index = losses.index(metric_value)
            metric = losses

        elif plot_metric == 'Accuracy':
            metric_value = max(accuracies)
            index = accuracies.index(metric_value)
            metric = accuracies

        initial_learning_rate = lrs[index]
        ...
        return ax, initial_learning_rate
